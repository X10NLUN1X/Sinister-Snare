...
import requests
import numpy as np
from config import UEX_API_BASE_URL, UEX_API_KEY
...
def get_live_data_from_api():
    ...
    """
    try:
        headers = {"Authorization": f"Bearer {UEX_API_KEY}"} if UEX_API_KEY else {}
        response = requests.get(UEX_API_BASE_URL, headers=headers, timeout=30)
